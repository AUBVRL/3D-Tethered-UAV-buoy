...
sysd = c2d(sys,Ts);
filters.Am_HPF = sysd.denominator{1};
filters.Bm_HPF = sysd.numerator{1};

% Low pass filter:
sys=tf([OMEGA_FILTER],[1 OMEGA_FILTER]);
sysd = c2d(sys,Ts);
filters.Am_LPF = sysd.denominator{1};
filters.Bm_LPF = sysd.numerator{1};

OMEGA_FILTER2 = 15;    % (rad/s) - For smoothing optitrack x and z data
% Low pass filter:
sys=tf([OMEGA_FILTER2],[1 OMEGA_FILTER2]);
sysd = c2d(sys,Ts);
filters.Am_LPF2 = sysd.denominator{1};
filters.Bm_LPF2 = sysd.numerator{1};
